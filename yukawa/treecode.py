...
from pylab      import *
from tree_utils import Cell, generateTree, getCoeff, getMultipole, upwardSweep, packData, computeIndices, precompute_terms, addSources, addSources2
from scipy.misc import factorial 
from P2Pwrap import direct_c, P2P_c
from M2Pwrap import M2P_c
import time
import sys
import cProfile
...
# Direct summation
tic = time.time()
direct_c(xi[0:sample_size],yi[0:sample_size],zi[0:sample_size],xj,yj,zj,phi_d[0:sample_size],phi_xd[0:sample_size],phi_yd[0:sample_size],phi_zd,m,kappa,eps)
...
if mode == 'cpu':
    tec = time.time()
    M2P_c(paux, pxaux, pyaux, pzaux, mpHost, xtHost, ytHost, ztHost, xcHost, ycHost, zcHost, offsetMltHost, offsetTarHost, sizeTarHost, P, kappa, int(Nm), index_large)
    tac = time.time()
    time_M2P = tac - tec
...
if mode == 'cpu':   
    tec = time.time()
    P2P_c(xtHost, ytHost, ztHost, xsHost, ysHost, zsHost, intPtrHost, offsetTarHost, sizeTarHost, offsetSrcHost, offsetIntHost, paux, pxaux, pyaux, pzaux, mHost, kappa, eps)
    tac = time.time()
    time_P2P = tac - tec
