           ...
           int *offTar, int offTarSize, int *sizeTar, int sizeTarSize, int P, REAL kappa, int Nm, int *Index, int indSize)
{
    ...
    #pragma omp parallel for private(CI_start, CI_end, CJ_start, CJ_end, sum_p, sum_px, sum_py, sum_pz, dx, dy, dz, a_aux, ax_aux, ay_aux, az_aux) shared(offTarSize, offTar, offMlt, x, y, z, xc, yc, zc, Nm, mult, sizeTar, p, px, py, pz)
	...
	int is[i_q];
	REAL ps[i_q], pxs[i_q], pys[i_q], pzs[i_q];
	int cont;
