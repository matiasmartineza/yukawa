...
# Wrapped code
from P2Pwrap import direct_c_cy
from calculateMultipoles import P2M_cy, M2M_cy
from M2Pwrap import getCoeff_cy, setIndex_cy, setIndex_arr_cy
    ...
    for ii in range(P+1):
        for jj in range(P+1-ii):
            for kk in range(P+1-ii-jj):
                index.append(setIndex_cy(P,ii,jj,kk))
                II.append(ii)
                JJ.append(jj)
                KK.append(kk)
...
#    index = setIndex_arr_cy(P,II,JJ,KK)
...
def getMultipole(Cells, C, x, y, z, m, twig, II, JJ, KK, index, P, NCRIT):
    ...
    if (Cells[C].ntarget>=NCRIT):
        ...
        '''
        l = Cells[C].source
        P2M_cy(Cells[C].M, x[l], y[l], z[l], m[l], Cells[C].xc, Cells[C].yc, Cells[C].zc, II, JJ, KK)
...
def precompute_terms(P, II, JJ, KK):
    ...
    for i in range(len(II)):
        ...
        setIndex_arr_cy(P, len(ii), index_aux, ii, jj, kk)
...
def upwardSweep(Cells, CC, PC, P, II, JJ, KK, Index, combII, combJJ, combKK, IImii, JJmjj, KKmkk, index, index_ptr):
    ...
    M2M_cy(Cells[PC].M, Cells[CC].M, dx, dy, dz, II, JJ, KK, combII, combJJ, combKK, IImii, JJmjj, KKmkk, index, index_ptr)
    ''' 
